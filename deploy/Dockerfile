...
RUN set -x \
    ...
    && apk add --no-cache --virtual .build-deps build-base g++ gcc libxslt-dev py3-lxml openldap-dev cyrus-sasl-dev python3-dev linux-headers \
...
RUN set -x \
    ...
    # 添加LDAP运行时库支持
    && apk add --no-cache libldap libsasl \
